...
def compute_project_readiness(workspace: ProjectWorkspace) -> ProjectReadiness:
    ...
    required_checks = [
        ...
        (candidate_count >= 2, LocalizedText(zh="至少选择 2 个候选 PLC 生态。", en="Select at least two candidate PLC ecosystems.")),
    ]
    recommended_checks = [
        ...
        (attachment_count > 0, LocalizedText(zh="登记至少一个附件信息记录。", en="Register at least one attachment information record.")),
        (
    ...
    reasons = [
        ...
        LocalizedText(
            zh=f"附件信息记录为 {attachment_count} 条；当前版本只登记文件信息，不读取或解析附件正文。",
            en=f"Attachment information record count is {attachment_count}; this version registers file information only and does not read or parse attachment contents.",
        ),
        LocalizedText(
            zh=f"Benchmark {'可生成' if benchmark_ready else '尚不可生成'}，报告状态为 {workspace.report.status}。",
            en=f"Benchmark is {'available' if benchmark_ready else 'not available yet'}; report status is {workspace.report.status}.",
        ),
    ]
    if workspace.project.status == "Finalized":
        reasons.append(LocalizedText(zh="项目已由用户确认最终版。", en="The project was explicitly marked Finalized by the user."))
...
def derive_project_status(
    ...
    *,
    ...
    if missing or readiness_score < 50 or candidate_count < 2:
        return "Draft"
    ...
    if workspace.report.status == "Ready" and report_sections_exist and has_report_body and benchmark_ready:
        return "Report Ready"
...
def _next_action(
    ...
    if not _benchmark_can_be_generated(workspace):
        return LocalizedText(
            zh="检查候选平台和偏好权重，使 benchmark 可以生成。",
            en="Check candidate platforms and preference weights so the benchmark can be generated.",
        )
    if workspace.report.status != "Ready":
        return LocalizedText(zh="完善或更新报告分区，使项目进入 Report Ready。", en="Complete or update report sections to reach Report Ready.")
    if recommended_missing:
        return recommended_missing[0]
    if workspace.project.status == "Finalized":
        return LocalizedText(zh="项目已确认最终版，可归档或导出报告。", en="The project is finalized and can be archived or exported.")
    ...
    for platform_id in workspace.intake.candidate_platforms:
        ...
        results.append(
            BenchmarkResult(
                ...
                assumptions=[
                    LocalizedText(zh="技术评分来自平台 scores 的简单平均值。", en="Technical score is the average of platform scores."),
                    ...
                    LocalizedText(zh="评分逻辑不调用 AI、不解析文件、不使用随机数。", en="Scoring does not call AI, parse files, or use randomness."),
                ],
            )
        )
