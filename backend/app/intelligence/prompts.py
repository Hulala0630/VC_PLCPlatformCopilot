...
def project_analysis_prompt(
    ...
    if request.focus == "attachments":
        ...
        return _bundle(
            "Produce a material readiness and decision-impact analysis for PLC platform selection and migration review. Use only attachment registration records: file name, file type, declared purpose, and upload date. State clearly that attachment file bodies have not been read or parsed. Structure the output around: Registered materials, What they can support, What they cannot support yet, Missing documents, Impact on benchmark confidence, Impact on report quality, and Decision next step. Explain what the registered materials may support, such as requirements validation, I/O scope framing, architecture review, electrical/safety readiness, migration risk review, benchmark confidence, and report drafting. Never infer, summarize, quote, or claim knowledge of Excel, PDF, drawing, or document contents.",
            context,
        )
...
def benchmark_explanation_prompt(
    ...
    context["requested_language"] = request.language
    return _bundle(
        "Produce an executive-style consulting decision analysis for PLC platform selection and migration review. Use the supplied project goal, industry, project size, I/O scale, motion requirement, safety requirement, budget sensitivity, team experience, existing platform, candidate platforms, platform preference weights and reasons, attachment registration records, fixed benchmark baseline, readiness, and status. Structure the output around: Recommendation, Why this platform, Alternative platform, Key risks, Preference impact, Missing confirmations, and Decision next step. Also cover the recommended platform, ranking rationale, technical fit analysis, business/preference impact, assumptions, uncertainty, missing inputs, and next recommended actions. Do not merely explain the scores; form a conclusion that can be used in an engineering review meeting. State that attachment file bodies have not been parsed. State that benchmark scores come from fixed benchmark calculation rules and that the advisory answer does not change scores or rankings. Do not recalculate, replace, tune, normalize, or propose changed scores or rankings. Keep the tone strategic, analytical, business-oriented, engineering-grounded, concise, and bilingual-compatible. Keep each paragraph self-contained for streaming display: conclusion first, then evidence, risks, assumptions, and actions.",
        context,
    )
...
def benchmark_analysis_prompt(
    ...
    context["requested_language"] = request.language
    return _bundle(
        "Produce a consulting decision analysis for automation engineers and technical managers. Use the supplied fixed benchmark baseline as the audit baseline. Return content that clearly covers Recommendation, Why this platform, Alternative platform, Key risks, Preference impact, Missing confirmations, and Decision next step. The analysis should be usable in a review meeting, not just an explanation of ranking values. You may recommend a candidate platform that is not the first-ranked baseline platform when migration continuity, installed base, team capability, downtime risk, lifecycle strategy, or business constraints justify it. If your advisory recommendation differs from the first-ranked baseline platform, explicitly explain the difference as a review item while preserving the supplied scores, ranking values, risk levels, and charts. Explain recommendation, risks, and assumptions without marketing language. Never change scores or ranking values, and do not expose internal implementation wording.",
        context,
    )
...
def report_generation_prompt(
    ...
    return _bundle(
        "Draft one formal PLC platform selection and migration consulting-report suggestion for every supplied report section. Preserve every supplied section_id and title exactly, in the same order. The report should read as a consultant report draft, not a chat reply and not a description of how output was generated. Across the supplied sections, cover Executive Summary, Decision Basis, Project Inputs, Platform Benchmark, Preference Impact, Risk Assessment, Implementation / Migration Roadmap, Assumptions & Uncertainty, Open Questions, and Review Notes. Use report-ready language for engineering managers: factual, concise, and decision-oriented. State that attachment file bodies have not been parsed when attachments are referenced. Do not change benchmark scores or rankings. Do not expose mock, placeholder, deterministic, provider, model, API, internal, or fallback wording.",
        context,
    )
